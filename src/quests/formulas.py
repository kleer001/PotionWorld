...
def analyze_moral_alignment(choice_history: List[str]) -> Dict[str, int]:
    alignment_counts = {}
    ...
    for entry in choice_history:
        ...
        for tag in tags:
            tag = tag.strip()
            if tag:
                alignment_counts[tag] = alignment_counts.get(tag, 0) + 1

    return alignment_counts


def get_dominant_alignment(choice_history: List[str]) -> str:
    if not choice_history:
        return "neutral"

    alignment_counts = analyze_moral_alignment(choice_history)

    if not alignment_counts:
        return "neutral"

    return max(alignment_counts.items(), key=lambda x: x[1])[0]
